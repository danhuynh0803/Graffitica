...
void graff::put_pixel(int x, int y, const color& _color, std::vector<std::vector<color> > &canvas)
{
...
void graff::swap(vec3 &p0, vec3 &p1) 
{
    vec3 temp(p0);
    p0 = p1; 
    p1 = temp; 
...
void graff::sort_desc(std::vector<vec3> &verts)
{
    for (int i = 0; i < verts.size(); ++i) 
    {
        for (int j = i + 1; j < verts.size(); ++j) 
        {
            ...
            {
                graff::swap(verts[i], verts[j]);
            }
        }
    }
}

void graff::draw_line(vec3 p0, vec3 p1, const color& _color, std::vector<std::vector<color> > &canvas)
{
    int dx = p1.x() - p0.x(); 
    int dy = p1.y() - p0.y(); 
    ...
    if (dx == 0 && dy == 0) 
    {
        graff::put_pixel(p0.x(), p0.y(), _color, canvas);
        return; 
    }
    ...
    if (abs(dx) > abs(dy))
    {
        ...
        {
            graff::put_pixel(x, (int)y, _color, canvas);
            y += m; 
        } 
    }
    ...
    else 
    {
        ...
        {
            graff::put_pixel((int)x, y, _color, canvas);
            x += m;
        }
    }
}
...
// ==========================================
void graff::fill_flat_bottom_triangle(vec3 p0, vec3 p1, vec3 p2, const color &_color, std::vector<std::vector<color> > &canvas)
{
    int dy = p0.y() - p1.y(); 
...
// precondition: p0.y == p1.y > p2.y
// =====================================
void graff::fill_flat_top_triangle(vec3 p0, vec3 p1, vec3 p2, const color &_color, std::vector<std::vector<color> > &canvas)
{
    int dy = p0.y() - p2.y();
    float slope_p2_p0 = (p0.x() - p2.x())/(p0.y() - p2.y());
...
// horizontally.
// =========================================
void graff::draw_triangle_filled(vec3 p0, vec3 p1, vec3 p2, const color &_color, std::vector<std::vector<color> > &canvas)
{
    // sort vertices on descending y 
    std::vector<vec3> verts = {p0, p1, p2};
    graff::sort_desc(verts);  
...
void graff::draw_triangle_wireframe(vec3 p0, vec3 p1, vec3 p2, const color &_color, std::vector<std::vector<color> > &canvas)
{
...
void graff::draw_triangle(vec3 p0, vec3 p1, vec3 p2, const color &_color, std::vector<std::vector<color> > &canvas, bool is_filled)
{
    if (is_filled)
    {
